...
import { XMarkIcon, EnvelopeIcon, ChatBubbleLeftRightIcon, PencilSquareIcon, StarIcon, ChatBubbleOvalLeftEllipsisIcon } from './icons.tsx';
...
const RequestDetailDrawer: React.FC<RequestDetailDrawerProps> = ({ request, currentUser, onClose, onUpdateRequest, onVote, onCommentSubmit }) => {
  ...
  const getRequestSourceIcon = (req: BMKRequest) => {
    ...
    return <PencilSquareIcon className="h-5 w-5 text-gray-400"/>;
  };
  ...
  return (
    <>
        <div 
            ...
            onClick={onClose}
        ></div>
        <div className={`fixed top-0 right-0 h-full bg-white/70 backdrop-blur-xl shadow-2xl w-full max-w-2xl z-40 transform transition-transform duration-300 ease-in-out ${request ? 'translate-x-0' : 'translate-x-full'}`}>
            <div className="flex flex-col h-full">
                <header className="p-6 border-b border-gray-200 flex justify-between items-start gap-4">
                    <div className="flex-1">
                         <div className="flex items-center gap-3 mb-2">
                            ...
                            <span className="font-semibold text-gray-500">{request.id}</span>
                        </div>
                        <input type="text" name="title" value={editedRequest.title} onChange={handleInputChange} className="text-2xl font-bold text-gray-900 bg-transparent w-full focus:outline-none focus:ring-0 border-0 p-0 -ml-1"/>
                    </div>
                    <div className="flex items-center gap-4 flex-shrink-0">
                         <button
                          onClick={() => onVote(request.id)}
                          className="flex items-center gap-1.5 text-gray-600 hover:text-orange-500 font-bold py-1 px-3 rounded-lg transition-colors bg-gray-100 hover:bg-orange/10"
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-8">
                     <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                         ...
                         <div>
                            <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider">Function</label>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6 border-t border-gray-200">
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-600 mb-1">Stakeholder</label>
                    ...
                    <div className="bg-gray-100/80 p-4 rounded-xl border border-gray-200/80">
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-base">
                            <div><span className="font-medium text-gray-500 block">Owner</span> {trackOwnerInfo.owner}</div>
                            <div><span className="font-medium text-gray-500 block">Contact</span> <a href={`mailto:${trackOwnerInfo.contact}`} className="text-blue hover:underline">{trackOwnerInfo.contact}</a></div>
                        </div>
                    </div>

                    <div>
                         <h3 className="font-semibold mb-3 flex items-center gap-3 text-gray-800 text-lg pt-6 border-t border-gray-200">
                         ...
                         <div className="space-y-4">
                            ...
                            <div className="flex items-start gap-3 pt-4 border-t border-gray-200/80">
                                <textarea 
                                    value={newComment}
                                ...
                                <button
                                    ...
                                    className="bg-blue text-white px-4 py-2 rounded-lg text-sm font-semibold hover:opacity-90"
                                >
                                    Post
                                </button>
                            </div>
                ...
                <footer className="p-4 bg-gray-50 border-t border-gray-200/80 flex justify-end gap-3">
                    <button onClick={onClose} className="bg-white py-2 px-4 border border-gray-300 rounded-lg shadow-sm text-sm font-medium text-gray-800 hover:bg-gray-50">
                        Cancel
                    </button>
                    <button onClick={handleUpdate} className="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-lg text-white bg-blue hover:opacity-90">
                        Save Changes
                    </button>
                </footer>
            </div>
